...
- **Phase 6c (core shaping validation): NO-GO (CONFIRMED by 6c-1)** — Band-shaped noise indistinguishable from uniform on 21 Cora subgraphs (p=0.748, mean delta=+3.05). Phase 6c-1's 30-subgraph rerun strengthens the negative conclusion (p=0.998, mean delta=+9.65).
...
- **Phase 6f (cross-protocol 2x2x2, 10 seeds): GO** — Phase 2g replicates cleanly (+11.7%, p=0.0001, 10/10 seeds). Phase 6e's specaug/specaug design rerun with 10 seeds shows +6.3%, p=0.013. Band shaping helps across all 4 train/ref protocol combos; training-data protocol modulates effect size (indep: +11.7%, specaug: +5-6%), evaluation-ref protocol has no effect.
- **Phase 6c-1 (H2 + power-boosted 6c): NO-GO + NO-GO** — Part A (synthetic community features on Cora topology, 10 subgraphs): -8.7%, p=0.898. Part B (real features + specaug, 30 subgraphs): -14.4%, p=0.998. Phase 2g's effect is narrow (SBM-specific topology). H2 rejected: shaping fails on Cora even with ideal synthetic features. Paper framing: **Framing B primary** (spectral augmentation from 6b, +46% W1); Framing A (shaping) is scope-bounded to synthetic block-structured graphs.
- **Phase 7-pre (continuous-feature sanity): NO-GO** — Cora-tuned pipeline does not denoise Elliptic Bitcoin (1/3 pass) or Stanford PPI (0/3 pass) at n=100 subgraphs; DDIM samples overshoot with std_ratio 10-215×. SVD d=16 captures 59% (Elliptic) / 40% (PPI) vs Cora's 15%, so reduction is healthy — the failure is in the generative pipeline's calibration on continuous features. Phase 7a/7b halted; pipeline repair required before re-attempting multi-dataset validation. Code shipped: `graph_fans/phase7/{real_dataset_loader,augmentation_baselines,downstream_classifier}.py`, `scripts/test_phase7pre_sanity.py`, `scripts/test_phase7a_multidataset.py`.
...
```bash
ssh -A anaderi@89.169.120.202  # NVIDIA L40S
